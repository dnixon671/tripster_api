...
export const registerUser = async (req, res) => {
    const { phone, password, role } = req.body; // Ahora el role viene del body
    ...
    try {
        ...
        // verificar que el role sea uno de los permitidos (client o driver)
        if (!["client", "driver"].includes(role)) {
            return res.status(400).json(
                {
                    success: false,
                    error: "Rol de usuario no es válido. Opciones permitidas: 'client', 'driver'"
                });
        }
        ...
        await changeIsOnline(user._id, true); // Cambiar el estado de conexión a true
...
export const loginUser = async (req, res) => {
    ...
    try {
        ...
        // Actualizar estado de conexión
        await changeIsOnline(user._id, true);
        ...
        res.status(200).json({
            ...
            token: _token  // Incluir token en la respuesta para la app móvil
        });
...
export const logoutUser = async (req, res) => {
    ...
    try {
        ...
        await changeIsOnline(req.user.id, false);
